...
with c4:
    ...
    ax.set_title("Earthquakes per Year", color="#e0e4f7", fontsize=12)
    st.pyplot(fig); plt.close(fig)
...
with c5:
    ...
    ax.set_title("Cumulative Events", color="#e0e4f7", fontsize=12)
    st.pyplot(fig); plt.close(fig)
...
with c6:
    ...
    ax.set_title("Average Magnitude per Year", color="#e0e4f7", fontsize=12)
...
with c7:
    ...
    ax.set_title("Depth vs Magnitude", color="#e0e4f7", fontsize=12)
    st.pyplot(fig); plt.close(fig)
...
sns.heatmap(corr_df, ax=ax, annot=True, fmt=".2f", cmap="coolwarm",
            ...
            cbar_kws={"shrink":0.8})
ax.set_title("Feature Correlation Matrix", color="#e0e4f7", fontsize=14, pad=12)
